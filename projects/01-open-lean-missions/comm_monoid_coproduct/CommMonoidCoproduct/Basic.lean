/-
  ...
  Mission: Formalization of the Coproduct Universal Property of Commutative Monoids in Lean 4
  Target: Standalone machine-checked Lean 4 formalization with 0 sorry and 0 custom axioms.
-/

namespace CommMonoidCoproductFormalization

/-- Monoid typeclass with associativity and two-sided identity laws. -/
...
/-- Binary direct product monoid instance. -/
...
/-- Commutative monoid instance on binary direct product M × N. -/
...
/-- Main Theorem 1: Left canonical coproduct inclusion homomorphism ι₁ : M → M × N. -/
...
/-- Main Theorem 2: Right canonical coproduct inclusion homomorphism ι₂ : N → M × N. -/
...
/-- Fundamental biproduct coordinate split: (m, n) = ι₁(m) · ι₂(n). -/
...
/-- Main Theorem 3: Categorical universal copairing map [f, g] : M × N → P. -/
...
/-- Main Theorem 4: First triangle commutation identity [f, g] ∘ ι₁ = f. -/
...
/-- Main Theorem 5: Second triangle commutation identity [f, g] ∘ ι₂ = g. -/
...
/-- Main Theorem 6: Categorical Coproduct Universal Uniqueness in CommMonoid. -/
theorem coprod_universal_unique {M N P : Type _} [MyCommMonoid M] [MyCommMonoid N] [MyCommMonoid P]
    ...
    h = copair f g := by
  ext p
  have h_split : p = (inlHom M N p.1) * (inrHom M N p.2) := prod_eq_inl_mul_inr p
  have h1 : h p = h ((inlHom M N p.1) * (inrHom M N p.2)) := congrArg h.toFun h_split
  calc
    h p = h ((inlHom M N p.1) * (inrHom M N p.2)) := h1
    _   = h (inlHom M N p.1) * h (inrHom M N p.2) := h.map_mul' (inlHom M N p.1) (inrHom M N p.2)
    _   = f p.1 * g p.2                           := by rw [h_inl p.1, h_inr p.2]
    _   = copair f g p                           := rfl

end CommMonoidCoproductFormalization
