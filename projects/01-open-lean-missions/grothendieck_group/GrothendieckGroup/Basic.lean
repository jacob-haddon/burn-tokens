/-
  ...
  Mission: Formalization of the Grothendieck Group Construction and Universal Property in Lean 4
  Target: Standalone machine-checked Lean 4 formalization with 0 sorry and 0 custom axioms.
-/

namespace GrothendieckGroupFormalization

/-- Commutative additive monoid typeclass. -/
...
/-- Additive abelian group typeclass. -/
...
/-- Additive monoid homomorphism structure. -/
...
/-- Main Definition 1: Grothendieck Equivalence Relation on M × M. -/
...
/-- Main Definition 2: Grothendieck Group Carrier Type as Quotient. -/
...
/-- Main Theorem 1: Grothendieck Group Forms an Additive Abelian Group. -/
...
/-- Main Theorem 2: Canonical Monoid Homomorphism ι : M → K(M). -/
...
theorem groth_lift_well_defined {M G : Type _} [MyAddCommMonoid M] [MyAddCommGroup G]
    (f : MyAddMonoidHom M G) (p1 p2 : M × M) (h : grothendieckRel p1 p2) :
    f p1.1 + -f p1.2 = f p2.1 + -f p2.2 := by
  obtain ⟨k, hk⟩ := h
...
/-- Main Theorem 3: Universal Grothendieck Group Lift Homomorphism. -/
def universalLift {M G : Type _} [MyAddCommMonoid M] [MyAddCommGroup G]
  ...
  toFun := Quotient.lift (fun (p : M × M) => f p.1 + -f p.2) (fun p1 p2 h => groth_lift_well_defined f p1 p2 h)
  map_zero' := by
    have h_zero : f (0 : M) + -f (0 : M) = (0 : G) := by
      rw [f.map_zero']
      calc
        (0 : G) + -(0 : G) = -(0 : G) + (0 : G) := add_comm_thm (0 : G) (-(0 : G))
        _ = (0 : G) := add_left_neg_thm (0 : G)
    exact h_zero
  map_add' a b := by
    induction a using Quotient.inductionOn with | h a =>
    induction b using Quotient.inductionOn with | h b =>
    show f (a.1 + b.1) + -f (a.2 + b.2) = (f a.1 + -f a.2) + (f b.1 + -f b.2)
...
/-- Main Theorem 4: Commutation Triangle universalLift f ∘ ι = f. -/
theorem universalLift_canonical {M G : Type _} [MyAddCommMonoid M] [MyAddCommGroup G]
  ...
  have h_ev : universalLift f (canonicalHom M m) = f m + -f (0 : M) := rfl
  rw [h_ev, f.map_zero']
  calc
    f m + -(0 : G) = f m + (0 : G) := by
      congr 1
      calc
        -(0 : G) = -(0 : G) + (0 : G) := (add_zero_thm (-(0 : G))).symm
        _ = (0 : G) := add_left_neg_thm (0 : G)
    _ = f m := add_zero_thm (f m)

/-- Main Theorem 5: Categorical Uniqueness of the Grothendieck Group Lift. -/
theorem universalLift_unique {M G : Type _} [MyAddCommMonoid M] [MyAddCommGroup G]
  ...
  have h_split : Quotient.mk (grothendieckSetoid M) p =
    ...
    rw [add_zero_thm, zero_add_thm, add_zero_thm, add_zero_thm p.1]
...
end GrothendieckGroupFormalization
