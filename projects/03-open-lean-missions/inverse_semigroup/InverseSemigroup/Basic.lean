...
theorem idempotent_left (x : G) : IsIdempotent (x * x⁻¹) := by
  dsimp [IsIdempotent]
  calc (x * x⁻¹) * (x * x⁻¹)
    _ = (x * x⁻¹ * x) * x⁻¹ := by rw [mul_assoc (x * x⁻¹), mul_assoc x x⁻¹ x]
    _ = x * x⁻¹             := by rw [mul_inv_self]
...
theorem idempotent_mul (e f : G) (he : IsIdempotent e) (hf : IsIdempotent f) :
  ...
  dsimp [IsIdempotent]
  calc (e * f) * (e * f)
    _ = e * (f * e) * f     := by rw [← mul_assoc, mul_assoc e (f * e) f, ← mul_assoc f e f, mul_assoc]
    _ = e * (e * f) * f     := by rw [idempotents_comm f e hf he]
    _ = (e * e) * (f * f)   := by rw [← mul_assoc e e f, mul_assoc (e * e), ← mul_assoc]
    _ = e * f               := by rw [he, hf]
...
theorem idempotent_conj (x : G) (f : G) (hf : IsIdempotent f) :
  ...
  calc (x * f * x⁻¹) * (x * f * x⁻¹)
    _ = x * f * (x⁻¹ * x) * f * x⁻¹ := by
      rw [← mul_assoc (x * f * x⁻¹), mul_assoc (x * f) x⁻¹ (x * f * x⁻¹),
          ← mul_assoc x⁻¹ (x * f * x⁻¹), mul_assoc x⁻¹ x (f * x⁻¹)]
    _ = x * (f * (x⁻¹ * x)) * f * x⁻¹ := by
      rw [← mul_assoc x f (x⁻¹ * x)]
    _ = x * ((x⁻¹ * x) * f) * f * x⁻¹ := by
      rw [idempotents_comm f (x⁻¹ * x) hf (idempotent_right x)]
    _ = x * (x⁻¹ * x) * (f * f) * x⁻¹ := by
      rw [mul_assoc x (x⁻¹ * x) f, ← mul_assoc (x * (x⁻¹ * x)) f f, mul_assoc]
    _ = (x * x⁻¹ * x) * f * x⁻¹       := by rw [hf, ← mul_assoc x x⁻¹ x]
    _ = x * f * x⁻¹                   := by rw [mul_inv_self]
...
theorem naturalLe_refl (x : G) : NaturalLe x x := by
  use (x * x⁻¹)
  exact ⟨idempotent_left x, by rw [← mul_assoc, mul_inv_self]⟩
...
theorem naturalLe_trans (x y z : G) (hxy : NaturalLe x y) (hyz : NaturalLe y z) :
  ...
  use (e * f)
  refine ⟨idempotent_mul e f he hf, ?_⟩
  rw [mul_assoc]
...
theorem naturalLe_mul_compat (x y u v : G) (h1 : NaturalLe x y) (h2 : NaturalLe u v) :
  ...
  use (e * (y * f * y⁻¹))
  refine ⟨idempotent_mul e (y * f * y⁻¹) he h_conj, ?_⟩
  calc e * (y * f * y⁻¹) * (y * v)
    _ = e * (y * f * (y⁻¹ * y)) * v := by
      rw [mul_assoc (e * (y * f * y⁻¹)) y v, ← mul_assoc e (y * f * y⁻¹) y,
          mul_assoc e (y * f * y⁻¹ * y), ← mul_assoc (y * f) y⁻¹ y, mul_assoc]
    _ = e * (y * ((y⁻¹ * y) * f)) * v := by
      rw [mul_assoc y f (y⁻¹ * y), idempotents_comm f (y⁻¹ * y) hf (idempotent_right y)]
    _ = e * ((y * y⁻¹ * y) * f) * v   := by
      rw [← mul_assoc y (y⁻¹ * y) f, mul_assoc y y⁻¹ y]
    _ = e * (y * f) * v               := by rw [mul_inv_self]
    _ = (e * y) * (f * v)             := by
      rw [← mul_assoc, mul_assoc e y (f * v), mul_assoc y f v]
...
theorem sigma_mul_left (x u v : G) (h : SigmaRel u v) : SigmaRel (x * u) (x * v) := by
  ...
  use (x * f * x⁻¹)
  refine ⟨idempotent_conj x f hf, ?_⟩
  calc (x * f * x⁻¹) * (x * u)
    _ = x * f * (x⁻¹ * x) * u := by
      rw [← mul_assoc (x * f * x⁻¹), mul_assoc (x * f) x⁻¹ (x * u),
          ← mul_assoc x⁻¹ (x * u), mul_assoc x⁻¹ x u]
    _ = x * (f * (x⁻¹ * x)) * u := by
      rw [← mul_assoc x f (x⁻¹ * x)]
    _ = x * ((x⁻¹ * x) * f) * u := by
      rw [idempotents_comm f (x⁻¹ * x) hf (idempotent_right x)]
    _ = (x * (x⁻¹ * x)) * (f * u) := by
      rw [mul_assoc x (x⁻¹ * x) (f * u), ← mul_assoc (x⁻¹ * x) f u, mul_assoc]
    _ = (x * x⁻¹ * x) * (f * u)   := by
      rw [mul_assoc x x⁻¹ x]
    _ = x * (f * u)               := by rw [mul_inv_self]
    _ = x * (f * v)               := by rw [huv]
    _ = (x * x⁻¹ * x) * (f * v)   := by rw [mul_inv_self]
    _ = (x * (x⁻¹ * x)) * (f * v) := by
      rw [mul_assoc x x⁻¹ x]
    _ = x * ((x⁻¹ * x) * f) * v   := by
      rw [mul_assoc x (x⁻¹ * x) (f * v), ← mul_assoc (x⁻¹ * x) f v, mul_assoc]
    _ = x * (f * (x⁻¹ * x)) * v   := by
      rw [idempotents_comm (x⁻¹ * x) f (idempotent_right x) hf]
    _ = x * f * (x⁻¹ * x) * v     := by
      rw [← mul_assoc x f (x⁻¹ * x)]
    _ = (x * f * x⁻¹) * (x * v)   := by
      rw [← mul_assoc (x * f * x⁻¹), mul_assoc (x * f) x⁻¹ (x * v),
          ← mul_assoc x⁻¹ (x * v), mul_assoc x⁻¹ x v]
...
theorem sigma_left_id (x y : G) : SigmaRel (x * x⁻¹ * y) y := by
  use (x * x⁻¹)
  refine ⟨idempotent_left x, ?_⟩
  calc (x * x⁻¹) * (x * x⁻¹ * y)
    _ = ((x * x⁻¹) * (x * x⁻¹)) * y := by rw [mul_assoc]
    _ = (x * x⁻¹) * y               := by rw [idempotent_left x]
