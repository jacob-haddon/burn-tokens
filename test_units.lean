...
theorem isUnit_inv {M : Type _} [MyMonoid M] {u : M} (h : IsUnit M u) :
    ∃ v : M, IsUnit M v ∧ u * v = 1 ∧ v * u = 1 := by
  rcases h with ⟨v, hv_r, hv_l⟩
  refine ⟨v, ⟨u, hv_l, hv_r⟩, hv_r, hv_l⟩

def MyUnits (M : Type _) [MyMonoid M] := { u : M // IsUnit M u }

def unitOne (M : Type _) [MyMonoid M] : MyUnits M :=
  ⟨1, isUnit_one M⟩

def unitMul {M : Type _} [MyMonoid M] (u1 u2 : MyUnits M) : MyUnits M :=
  ⟨u1.1 * u2.1, isUnit_mul u1.2 u2.2⟩

noncomputable def unitInv {M : Type _} [MyMonoid M] (u : MyUnits M) : MyUnits M :=
  let v := Classical.choose u.2
  have hv := Classical.choose_spec u.2
  ⟨v, ⟨u.1, hv.2, hv.1⟩⟩

theorem unitInv_val_right {M : Type _} [MyMonoid M] (u : MyUnits M) :
    u.1 * (unitInv u).1 = 1 :=
  (Classical.choose_spec u.2).1

theorem unitInv_val_left {M : Type _} [MyMonoid M] (u : MyUnits M) :
    (unitInv u).1 * u.1 = 1 :=
  (Classical.choose_spec u.2).2

@[ext]
theorem MyUnits.ext {M : Type _} [MyMonoid M] {u1 u2 : MyUnits M} (h : u1.1 = u2.1) : u1 = u2 :=
  Subtype.ext h
...
instance instMyMonoidUnits (M : Type _) [MyMonoid M] : MyMonoid (MyUnits M) where
  ...
  mul_assoc a b c := by
    apply MyUnits.ext
    dsimp [unitMul]
    exact mul_assoc_thm a.1 b.1 c.1
  one_mul a := by
    apply MyUnits.ext
    dsimp [unitMul, unitOne]
    exact one_mul_thm a.1
  mul_one a := by
    apply MyUnits.ext
    dsimp [unitMul, unitOne]
    exact mul_one_thm a.1

noncomputable instance instMyGroupUnits (M : Type _) [MyMonoid M] : MyGroup (MyUnits M) where
  inv := unitInv
  inv_mul_cancel a := by
    apply MyUnits.ext
    dsimp [instMyMonoidUnits, unitMul, unitOne]
...
theorem unit_inv_inv {M : Type _} [MyMonoid M] (u : MyUnits M) :
  ...
  apply MyUnits.ext
  dsimp [unitInv]
  have h1 : (unitInv (unitInv u)).1 * (unitInv u).1 = 1 := unitInv_val_left (unitInv u)
  have h2 : (unitInv u).1 * (unitInv (unitInv u)).1 = 1 := unitInv_val_right (unitInv u)
  have hu1 : u.1 * (unitInv u).1 = 1 := unitInv_val_right u
  have hu2 : (unitInv u).1 * u.1 = 1 := unitInv_val_left u
  exact unit_inv_unique (unitInv u).1 (unitInv (unitInv u)).1 u.1 ⟨h2, h1⟩ ⟨hu2, hu1⟩
...
theorem unit_inv_mul {M : Type _} [MyMonoid M] (u1 u2 : MyUnits M) :
  ...
  have h_prod_r : (u1.1 * u2.1) * ((unitInv u2).1 * (unitInv u1).1) = 1 := by
    calc
      (u1.1 * u2.1) * ((unitInv u2).1 * (unitInv u1).1)
        = u1.1 * (u2.1 * ((unitInv u2).1 * (unitInv u1).1)) := mul_assoc_thm u1.1 u2.1 _
      _ = u1.1 * ((u2.1 * (unitInv u2).1) * (unitInv u1).1) := by rw [mul_assoc_thm u2.1 (unitInv u2).1 _]
      _ = u1.1 * (1 * (unitInv u1).1)                       := by rw [unitInv_val_right u2]
      _ = u1.1 * (unitInv u1).1                             := by rw [one_mul_thm (unitInv u1).1]
      _ = 1                                                 := unitInv_val_right u1
  have h_prod_l : ((unitInv u2).1 * (unitInv u1).1) * (u1.1 * u2.1) = 1 := by
    calc
      ((unitInv u2).1 * (unitInv u1).1) * (u1.1 * u2.1)
        = (unitInv u2).1 * ((unitInv u1).1 * (u1.1 * u2.1)) := mul_assoc_thm _ _ _
      _ = (unitInv u2).1 * (((unitInv u1).1 * u1.1) * u2.1) := by rw [mul_assoc_thm _ u1.1 _]
      _ = (unitInv u2).1 * (1 * u2.1)                       := by rw [unitInv_val_left u1]
      _ = (unitInv u2).1 * u2.1                             := by rw [one_mul_thm u2.1]
      _ = 1                                                 := unitInv_val_left u2
  have h_inv_r : (u1.1 * u2.1) * (unitInv (unitMul u1 u2)).1 = 1 := unitInv_val_right (unitMul u1 u2)
  have h_inv_l : (unitInv (unitMul u1 u2)).1 * (u1.1 * u2.1) = 1 := unitInv_val_left (unitMul u1 u2)
  exact (unit_inv_unique (u1.1 * u2.1) (unitInv (unitMul u1 u2)).1 ((unitInv u2).1 * (unitInv u1).1)
...
def mapUnits {M N : Type _} [MyMonoid M] [MyMonoid N]
  ...
  toFun u := ⟨f u.1, map_isUnit f u.1 u.2⟩
  map_one' := by
    apply MyUnits.ext
    dsimp
    exact f.map_one'
