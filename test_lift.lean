...
theorem universalLift_unique {M G : Type _} [MyAddCommMonoid M] [MyAddCommGroup G]
  ...
  have h_split : Quotient.mk (grothendieckSetoid M) p =
    ...
    apply Quotient.sound
    refine ⟨0, ?_⟩
    show p.1 + (0 + p.2) + 0 = (p.1 + 0) + 0 + 0
    rw [zero_add_thm, add_zero_thm]
    calc
      p.1 + p.2 + 0 = p.1 + p.2 := add_zero_thm _
      _ = (p.1 + 0) + p.2 := by rw [add_zero_thm]
      _ = (p.1 + 0) + p.2 + 0 := (add_zero_thm _).symm
      _ = (p.1 + 0) + (p.2 + 0) := by rw [add_assoc_thm]
      _ = (p.1 + 0) + (0 + p.2) := by rw [add_comm_thm p.2 0]
      _ = (p.1 + 0) + 0 + p.2 := by rw [← add_assoc_thm]
      _ = (p.1 + 0) + 0 + 0 := by sorry
  sorry
